...
public class ShipperUpdatedEventHandler : IDomainEventHandler<EntityUpdatedEvent<Shippers>>
{
    private readonly IRepository<AuditLog, string> _auditLogRepository;
    private readonly ICurrentUser _currentUser;
    private readonly UserService _userService;

    public ShipperUpdatedEventHandler(
    ...
    public async Task HandleAsync(EntityUpdatedEvent<Shippers> domainEvent, CancellationToken cancellationToken = default)
    {
        var auditLog = new AuditLog
        {
            Id = UlidExtension.Generate(),
            UserId = _currentUser.UserId,
            Action = EventTypeConstants.ShipperUpdated,
